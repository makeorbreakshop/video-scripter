...
async function calculateCentroids() {
  ...
  while (true) {
    const { data: videos, error } = await supabase
      ...
      .gte('topic_cluster_id', 0)  // Exclude outliers (-1)
  ...
  console.log(`Found ${videos.length} videos with topic assignments`);
  ...
  for (const [topicId, topicVideos] of Object.entries(videosByTopic)) {
    ...
    for (let i = 0; i < topicVideos.length; i += batchSize) {
      ...
      try {
        ...
        console.error(`Error fetching title embeddings for batch:`, err);
      }
      ...
      try {
        ...
        console.error(`Error fetching summary embeddings for batch:`, err);
      }
  ...
  const outputPath = './bertopic_centroids_20250803.json';
  ...
  stats.sort((a, b) => b.videos - a.videos).slice(0, 10).forEach(s => {
