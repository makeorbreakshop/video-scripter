...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.$$eval('[data-cs-thumb]',nodes=>nodes.length),1,'Video preview must include its thumbnail');
    await page.waitForFunction(()=>{const img=document.querySelector('[data-cs-thumb]');return img?.complete && img.naturalWidth>0;},{timeout:10000});
    ...
    assert(!text.includes('Comparison and forecast'),'No standalone broadcast explanation');
    assert(!text.includes('Views since stream start'),'No repeated chart heading');
    const controls=await page.$eval('button',e=>e.getBoundingClientRect().top);
    const headerBottom=await page.$eval('.vp-head',e=>e.getBoundingClientRect().bottom);
    assert(controls-headerBottom < 40,'Chart controls should directly follow compact metadata');
