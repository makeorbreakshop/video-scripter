...
/**
 * Fix ALL Temporal Baselines with CORRECT Calculation
 * 
 * Based on the actual implementation from August 7-8:
 * 1. Get last 10 videos from channel
 * 2. Only use videos that were 30+ days old at publication time
 * 3. For those videos, get their Day 30 views (using backfill if needed)
 * 4. Take the AVERAGE (not median) as the baseline
 * 
 * This bypasses the broken database function entirely
 */
...
async function fixAllBaselines() {
  ...
  try {
    ...
    console.log(`Current median baseline: ${colors.red}${parseFloat(stats.rows[0].median_baseline).toFixed(2)}${colors.reset} (way too high!)`);
    ...
    for (let i = 0; i < allVideos.rows.length; i++) {
      ...
      if (channelHistory.rows.length >= 3) {
        ...
        if (day30Views.length > 0) {
          // Calculate AVERAGE (not median!)
          baseline = day30Views.reduce((sum, val) => sum + val, 0) / day30Views.length;
        }
      } else if (channelHistory.rows.length > 0) {
        // Less than 3 videos, but at least 1 - still calculate average
        const day30Views = [];
    ...
    console.log(`Median baseline: ${colors.green}${parseFloat(after.median_baseline).toLocaleString()}${colors.reset} views (should be much lower now!)`);
    ...
    console.log(`3. AVERAGE (not median) as the baseline`);
