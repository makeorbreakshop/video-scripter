...
- Shared model observation SQL and the legacy benchmark exclude model-ineligible/conflicted rows. Replay also censors by received_at, so it cannot see a response before arrival. Observation version is `v5.2-rss-response-time`; formula and fit unchanged. Existing bounded scorer runs refresh version-mismatched rows; no forced full-corpus rescore was run.
...
## Rollout handoff
1. Review this branch alongside concurrent scoring work before integration; another task is developing a separate cadence-related V5.2 model change. Reconcile shared scoring/query/version changes at merge time.
