...
// Read batch 10
const batch10 = JSON.parse(fs.readFileSync('docs/comment-batches/batch-10.json', 'utf8'));

console.log(`Batch 10 contains ${batch10.length} comments`);

// Check date range
const dates = batch10.map(c => c.published_at);
const earliest = dates.sort()[0];
const latest = dates.sort()[dates.length - 1];
console.log(`Date range: ${earliest} to ${latest}`);

// Extract key fields for analysis
const extractedData = batch10.map(comment => ({
...
// Save the splits
fs.writeFileSync('batch-10-part1.json', JSON.stringify(firstHalf, null, 2));
fs.writeFileSync('batch-10-part2.json', JSON.stringify(secondHalf, null, 2));

// File size check
const part1Size = fs.statSync('batch-10-part1.json').size / 1024;
const part2Size = fs.statSync('batch-10-part2.json').size / 1024;
...
const businessCount = batch10.filter(c => 
  businessTerms.some(term => c.comment_text.toLowerCase().includes(term))
).length;

const painCount = batch10.filter(c => 
...
batch10.forEach(c => {
  ...
  if (lower.includes('fiber')) mentions.fiber++;
});

console.log(`\nQuick patterns:`);
console.log(`- Year: ${yearMatch}`);
console.log(`- Business intent: ${businessCount} comments (${(businessCount/batch10.length*100).toFixed(1)}%)`);
console.log(`- Pain/problems: ${painCount} comments (${(painCount/batch10.length*100).toFixed(1)}%)`);
