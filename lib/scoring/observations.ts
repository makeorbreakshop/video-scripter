...
export const OBSERVATION_SCORE_VERSION = 'v5.2-rss-response-time';
export type ObservationPoint = { at: string | Date; views: number; timeBasis?: string; receivedAt?: string | Date };
export type Observation = { day: number; views: number; source: 'snapshot' | 'sample' | 'rss'; at: string; timeBasis?: string; receivedAt?: string | Date };
...
export function mergeObservations(
  ...
  for (const [points, source] of [[samples, 'sample'], [rss, 'rss'], [snapshots, 'snapshot']] as const) {
    for (const p of points) {
      if (p.receivedAt && !(new Date(p.receivedAt).getTime() <= asOf)) continue;
      ...
      if (!existing || rank[source] > rank[existing.source]) byTime.set(ms, { at, views, source, day: (ms - published) / 86400000, ...(p.timeBasis ? { timeBasis: p.timeBasis, receivedAt: p.receivedAt } : {}) });
    }
  }
...
export const OBSERVATION_RECORDS_SQL = `
  ...
  select x.video_id, x.at, x.views, x.source, x.received_at, v.published_at
  from (
    select video_id, snapshot_date::timestamptz + interval '12 hours' as at, view_count as views, 'snapshot' as source, null::timestamptz as received_at
    from view_snapshots where video_id = any($1)
    union all
    select video_id, sampled_at, view_count, 'sample', null::timestamptz from view_samples where video_id = any($1)
    union all
    select video_id, at, views, 'rss', received_at from rss_samples where video_id = any($1) and model_eligible and not conflicted
  ) x join target_videos v on v.id = x.video_id
  where x.views >= 0 and x.at >= v.published_at and x.at <= now()`;

export function observationRecords(rows: { video_id: string; published_at: string | Date; at: string | Date; views: number; source: Observation['source']; received_at?: string | Date }[], asOf = Date.now()): Map<string, Observation[]> {
  ...
  for (const r of rows) {
    ...
    grouped.get(r.video_id)![r.source].push({ at: r.at, views: Number(r.views), receivedAt: r.received_at });
  }
