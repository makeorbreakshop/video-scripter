...
export interface BackfillChannel { channelId: string; cursor: BackfillCursor | null }
...
export const unfinishedUserUploadChannelsSql = (limit: number) => `
  select ct.channel_id,
    ...
    left join feed_watermarks w
      on w.source = '${USER_UPLOAD_BACKFILL_PREFIX}' || ct.channel_id
   where ct.lane = 'user'
     and ct.backfill_status = 'done'
...
export const userUploadBackfillPageSql = (hasCursor: boolean, limit: number, longform: string) => `
