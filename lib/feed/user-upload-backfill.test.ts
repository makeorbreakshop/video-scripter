...
test('pages multiple unfinished channels fairly within a global row bound', async () => {
  const channels = ['a', 'b', 'c'].map(channelId => ({ channelId, cursor: null }));
  const seen: string[] = [];
...
test('keeps the exact timestamp text and id tie-breaker when resuming', async () => {
  ...
  const [unit] = await buildUserUploadBackfillUnits({ channels: [{ channelId: 'a', cursor: null }], pageSize: 2, globalLimit: 2,
    fetchPage: async () => [row('z', precise), row('a', precise)],
  });
  expect(unit.cursor).toEqual({ at: precise, id: 'a' });
  const sql = userUploadBackfillPageSql(true, 2, 'true');
...
test('marks a short or empty page complete and retrying starts from the saved cursor', async () => {
  ...
  const [unit] = await buildUserUploadBackfillUnits({ channels: [{ channelId: 'a', cursor }], pageSize: 3, globalLimit: 3,
...
test('stops before fetching another channel when aborted', async () => {
  let calls = 0;
  const units = await buildUserUploadBackfillUnits({ channels: [{ channelId: 'a', cursor: null }, { channelId: 'b', cursor: null }], pageSize: 2, globalLimit: 4,
...
test('completed channels are excluded while absent newly tracked channels sort first', () => {
  ...
  expect(sql).toContain("ct.backfill_status = 'done'");
...
test('waits for catalog completion so a cursor cannot step past history still being imported', () => {
  expect(unfinishedUserUploadChannelsSql(10)).toMatch(/backfill_status\s*=\s*'done'/);
});
