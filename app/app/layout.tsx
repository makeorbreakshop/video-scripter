...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const user = await requireAppUser().catch((e) => {
  ...
  return (
    <div className={`${inter.variable} ${mono.variable} ${pixel.variable} cs-app`}>
      <Preconnects />
      <AppShell showInspiration={canSeeInspiration(user)} showAudience={isOwner(user)} showOutliers={isOwner(user)} showAngles={isOwner(user)}>{children}</AppShell>
    </div>
  );
}
