...
// The Outliers filter row, read left to right as three questions of falling importance:
//
//   WHERE   Near / Adjacent / Far — the one axis this page exists for, so it leads the line as
//           chips, the loudest control the system has.
//   WHEN    7d / 30d / 90d — same shape, lower stakes, so it sits after a hairline rather than
//           in a second row: same treatment, different group.
//   HOW STRICT + order — pushed to the right edge behind two menu triggers. The three quality
//           guards default sensibly and are usually left alone, so they collapse into one plate
//           whose trigger still reads out its own values (2x · Confirmed, Likely · 500+) and
//           carries a badge when any of them is off the default. Nothing is hidden, only folded.
//
...
const SORTS: Array<[OutlierSort, string]> = [['score', 'Top score'], ['published', 'Newest']];
