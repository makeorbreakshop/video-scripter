...
export async function POST(request: NextRequest) {
  try {
    ...
    // Process channels in parallel batches to optimize performance
    const BATCH_SIZE = 3; // Process 3 channels simultaneously to avoid API rate limits
...
async function processChannel(channelId: string, apiKey: string, maxResults: number, publishedAfter: string | null, excludeShorts: boolean, userId: string, results: any) {
  try {
    ...
    // Atomic updates to shared results object
