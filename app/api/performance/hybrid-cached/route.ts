...
import { getCachedHybridPerformance, refreshChannelPerformanceMetrics } from '@/lib/hybrid-performance-cached';
...
export async function GET(request: Request) {
  ...
  try {
    // Optionally refresh the metrics first
    if (refresh) {
      await refreshChannelPerformanceMetrics(channelName);
    }
    
    // Get cached performances
    const performances = await getCachedHybridPerformance(channelName, limit, timeframe);
    ...
    return NextResponse.json({
      ...
      videos: performances,
      cached: !refresh,
      message: refresh ? 'Data refreshed from source' : 'Using cached data'
    });
  } catch (error) {
    console.error('Error getting cached hybrid performance:', error);
    return NextResponse.json(
