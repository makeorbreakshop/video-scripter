-- Fix packaging performance function - correct filtering and individual baselines
...
BEGIN
  ...
  -- Execute query with corrected filtering and individual rolling baselines
  RETURN QUERY EXECUTE format('
    WITH filtered_videos AS (
      SELECT 
        ...
        v.is_competitor,
        v.channel_id,
        v.rolling_baseline_views as channel_avg_views,  -- Show individual rolling baseline, not channel average
        COUNT(*) OVER() as total_count
      FROM videos v
      WHERE (v.duration IS NULL OR v.duration = ''P0D'' OR v.duration ~ ''^PT[0-5]?[0-9]S$'') = FALSE  -- Only exclude actual shorts
        AND %s  -- date filter
