...
-- Update packaging performance function to use pre-calculated rolling baselines
  ...
  CASE p_sort_by
    WHEN 'performance_ratio' THEN 
      IF p_sort_order = 'desc' THEN
        sort_condition := 'CASE WHEN v.rolling_baseline_views > 0 THEN (v.view_count::float / v.rolling_baseline_views) END DESC NULLS LAST';
      ELSE
        sort_condition := 'CASE WHEN v.rolling_baseline_views > 0 THEN (v.view_count::float / v.rolling_baseline_views) END ASC NULLS LAST';
      END IF;
    WHEN 'view_count' THEN sort_condition := 'v.view_count ' || p_sort_order;
    WHEN 'published_at' THEN sort_condition := 'v.published_at ' || p_sort_order;
    ELSE sort_condition := 'v.published_at DESC';
  END CASE;
  
  -- Execute query with pre-calculated baselines
  RETURN QUERY EXECUTE format('
    WITH filtered_videos AS (
      ...
      WHERE v.duration IS NOT NULL 
        AND v.duration > 60  -- Exclude shorts
        AND %s  -- date filter
