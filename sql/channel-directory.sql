...
-- Earlier builds made this a materialized view; retire it before creating the table.
drop materialized view if exists channel_directory;
...
create or replace function refresh_channel_directory() returns integer language sql as $fn$
  with upserted as (
    insert into channel_directory (channel_id, name, norm, handle, avatar_url, video_count, tracked_lane, refreshed_at)
    select channel_id, name, norm, handle, avatar_url, video_count, tracked_lane, now() from (
      with vid as (
