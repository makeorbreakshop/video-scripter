...
async function renderCandidates() {
  try {
    ...
    document.getElementById('cands').innerHTML = rows
      .map((r) => `<div>${r.status === 'enrolled' ? '\u2705' : '\u{1F50E}'} <span class="nm">${r.channel_title}</span> <span style="color:#888">${(r.subscriber_count/1000).toFixed(0)}K subs \u00b7 seen ${r.seen_count}\u00d7</span> <span style="color:#6a6">${r.status}</span></div>`)
