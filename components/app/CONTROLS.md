...
| Menu | popover of choices anchored to a trigger | `<Menu>` — `components/app/menu.tsx` | one plate, 10px radius, 40px rows; `mode="multi"` tri-state, `mode="actions"` plain/destructive, `mode="filters"` several small value groups in one plate (`section` heads each, checkmark rows, panel stays open) |
