...
import { useThemeColors, fmtViews, HoverCard } from './video-chart';
import { localDay, localDayYear } from '@/lib/app/local-time';
...
// Labels are in the READER's zone (lib/app/local-time), never Eastern.
const monthYear = (t: number) => new Date(t).toLocaleDateString('en-US', { month: 'short', year: '2-digit' });
const axisTick = (t: number, fmt: 'month' | 'day') => (fmt === 'month' ? monthYear(t) : localDay(t));
    ...
    >
      ...
      {hovered && (
        ...
        >
          ...
          <div style={{ color: C.muted }}>{localDayYear(hovered.t)}</div>
