 ...
 */
function generateSearchAngles(concept, hook, frame) {
    const angles = {
 ...
 */
async function searchTitles(query, topK = 30, minScore = 0.3) {
    try {
 ...
 */
async function searchSummaries(query, topK = 20, minScore = 0.25) {
    try {
        const embedding = await openai.embeddings.create({
            model: 'text-embedding-3-small',
            input: query,
 ...
 */
async function findCrossNicheHighPerformers(concept, minTPS = 2.5) {
 ...
 */
async function getTopPerformers(minTPS = 3.0, limit = 10) {
 ...
 */
async function findChannelGaps(channelId) {
 ...
 */
async function enrichResults(pineconeMatches, source, query) {
    ...
    const allVideos = await Promise.all(batches.map(async (batch) => {
        const { data } = await supabase
            .from('videos')
            .select('id, title, channel_name, channel_id, thumbnail_url, view_count, published_at, temporal_performance_score, performance_ratio, topic_niche, format_type')
            .in('id', batch);
 ...
 */
export async function explorePatternsTool(params) {
    const { core_concept, current_hook, frame, channel_id, exploration_depth = 3, min_performance = 1.5 } = params;
    ...
    try {
        ...
        for (const [angleType, query] of anglesToUse) {
            searchPromises.push(searchTitles(query, 10, 0.35).then(matches => ({
                type: 'title',
                angleType,
                query,
                matches
        ...
        if (firstAngle) {
            searchPromises.push(searchSummaries(firstAngle[1], 10, 0.3).then(matches => ({
                type: 'summary',
        ...
        searchPromises.push(findCrossNicheHighPerformers(core_concept, min_performance).then(videos => ({
            type: 'cross_niche',
            videos
        })));
        // Top performers - reduced limit
        searchPromises.push(getTopPerformers(min_performance * 2, 5).then(videos => ({
            type: 'top_performers',
            videos
        })));
        // Channel gaps if channel_id provided
        if (channel_id) {
            searchPromises.push(findChannelGaps(channel_id).then(videos => ({
                type: 'channel_gaps',
                videos
            })));
        }
...
//# sourceMappingURL=explore-patterns.js.map