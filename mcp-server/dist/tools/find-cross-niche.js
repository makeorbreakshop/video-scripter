 ...
 */
export async function findCrossNichePatternsTool(params) {
    const { psychological_trigger, exclude_niches = [], min_performance = 2.0, limit = 30 } = params;
    ...
    try {
        ...
        videos?.forEach(video => {
            ...
            if (exclude_niches.includes(niche))
                return;
        ...
        const response = {
            ...
            results: {
                by_niche: Object.fromEntries(Array.from(nicheGroups.entries())
                    .map(([niche, videos]) => [
                    niche,
...
//# sourceMappingURL=find-cross-niche.js.map