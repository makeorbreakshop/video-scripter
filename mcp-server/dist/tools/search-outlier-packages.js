...
export function parseSearchOutlierPackagesParams(value) {
...
export function inferPackageFamily(title) {
...
export function selectCrossTopicCandidatePool(candidates, semanticallyNearIds, excludeChannelId, limit, minBaseline = 5_000, minOutlierScore = 2) {
    const unique = new Map();
    for (const candidate of candidates) {
        if (!candidate.id || unique.has(candidate.id) || semanticallyNearIds.has(candidate.id))
            continue;
        if (excludeChannelId && candidate.channel_id === excludeChannelId)
            continue;
        if (candidate.score < minOutlierScore || candidate.n_baseline < 10 || (candidate.baseline ?? 0) < minBaseline
            || !['likely', 'confirmed'].includes(candidate.confidence))
            continue;
        if (!candidate.title.trim() || !candidate.description.trim() || !candidate.thumbnail_url.trim())
            continue;
    ...
    while (output.length < limit) {
        ...
        for (const [family, group] of orderedGroups) {
            ...
            while (cursor < group.length && channels.has(group[cursor].channel_id))
                cursor += 1;
...
export function applyTransferGuard(target, result) {
    ...
    for (const number of numbersIn(proposedText)) {
        if (!allowedNumbers.has(number))
            issues.add(`unsupported_number:${number}`);
    }
    ...
    for (const [issue, pattern] of unsupportedClaims) {
        if (pattern.test(proposedText) && !pattern.test(targetText))
            issues.add(issue);
    }
    const constraints = (target.hard_constraints ?? []).join('\n').toLocaleLowerCase('en-US');
    const constrainedClaims = [
    ...
    for (const [issue, constraintPattern, proposedPattern] of constrainedClaims) {
        if (constraintPattern.test(constraints) && proposedPattern.test(proposedText))
            issues.add(issue);
    }
    for (const flag of result.risk_flags)
        issues.add(`model_risk:${flag}`);
    let status;
...
function sourceShape(candidate) {
    return {
        id: candidate.id,
...
function targetSearchDocument(target, explicitQuery) {
    return explicitQuery ?? `${target.title}\n${target.description.slice(0, 2_000)}`;
}
function diversifyTopicResults(candidates, limit, minOutlierScore, minBaseline) {
    const output = [];
    const channels = new Set();
    const seen = new Set();
    for (const candidate of candidates) {
        if (seen.has(candidate.id) || channels.has(candidate.channel_id))
            continue;
...
function transferRank(result, source) {
...
export async function runOutlierPackageSearch(rawParams, dependencies = productionDependencies()) {
    ...
    if (params.search_intent === 'topic') {
        ...
        const results = diversifyTopicResults(hits, params.top_k, params.min_outlier_score, params.min_baseline).map((candidate, index) => ({
            rank: index + 1,
    ...
    const preselected = selectCrossTopicCandidatePool(corpus, nearIds, params.exclude_channel_id, Math.min(Math.max(params.candidate_pool_size * 8, 16), 128), params.min_baseline, params.min_outlier_score);
    if (!preselected.length)
        throw new Error('No guarded outliers remained after cross-topic filtering');
    ...
    if (!withImages.length)
        throw new Error('No cross-topic outlier thumbnails were available for package analysis');
    ...
    const modelUsage = { ...shortlist.usage };
    let transferAnalysisCost = 0;
    for (let index = 0; index < withImages.length; index += GEMINI_BATCH_SIZE) {
        if (geminiCost + GEMINI_BATCH_BUDGET_RESERVE_USD > params.max_cost_usd)
            break;
    ...
    return {
        ...
        receipt: {
            ...
            guard_status_counts: guarded.reduce((counts, { transfer }) => {
...
export async function searchOutlierPackagesTool(params, dependencies) {
    const response = await runOutlierPackageSearch(params, dependencies);
    return { content: [{ type: 'text', text: JSON.stringify(response, null, 2) }] };
}
function candidateFromPoint(point) {
    const payload = point.payload;
    const id = payload.video_id || payload.entity_id;
    if (!id || !payload.title || !payload.channel_id || !payload.channel_name)
        return null;
    const score = Number(payload.score);
    const nBaseline = Number(payload.n_baseline);
    if (!Number.isFinite(score) || !Number.isFinite(nBaseline))
        return null;
    return {
        id,
        title: payload.title,
...
function qdrantFilter(minOutlierScore, minBaseline, excludeChannelId) {
    return {
        must: [
...
async function qdrantSemanticSearch(collection, vector, options) {
    ...
    return (response.result?.points ?? []).flatMap((point) => {
...
async function qdrantOutlierScan(collection, options) {
    const output = [];
    let offset;
...
async function loadThumbnail(url) {
    try {
        ...
        if (parsed.protocol !== 'https:' || !allowedThumbnailHost(parsed.hostname))
            return null;
        const response = await fetch(parsed, { signal: AbortSignal.timeout(8_000), redirect: 'follow' });
        if (!response.ok || !allowedThumbnailHost(new URL(response.url).hostname))
            return null;
...
function estimatedGeminiCost(usage) {
...
async function shortlistWithGemini(input) {
    const apiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY;
    if (!apiKey)
        throw new Error('GEMINI_API_KEY or GOOGLE_API_KEY is required for package_transfer');
    ...
    const output = raw.candidates?.[0]?.content?.parts
        ...
        .join('');
    if (!output)
        throw new Error('Gemini returned no structured package shortlist');
    const parsed = JSON.parse(output);
    if (!Array.isArray(parsed.results))
        throw new Error('Gemini package shortlist omitted results');
...
async function analyzeWithGemini(input) {
    ...
    const output = raw.candidates?.[0]?.content?.parts
        ...
        .join('');
    if (!output)
        throw new Error('Gemini returned no structured package analysis');
    const parsed = JSON.parse(output);
    if (!Array.isArray(parsed.results))
        throw new Error('Gemini package analysis omitted results');
    const usage = numericUsage(raw.usageMetadata);
    return {
        results: parsed.results,
        model: raw.modelVersion ?? model,
...
//# sourceMappingURL=search-outlier-packages.js.map